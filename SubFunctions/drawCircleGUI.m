function [img] = drawCircleGUI(axes,handles,eventdata,hObject)
...
%     cp(1,:)
    if isempty(p)
      ...
      color = str{val};
      circCent = [P(1,1) P(1,2)];
      img = getimage(handles.axesCamera1);
      imgOrig = img;
      img = insertShape(img,'FilledCircle',[circCent 1],'Color',color,'Opacity',0.3);
...
%       circCent = [P(1,1) P(1,2)];
      circBord = cp;
      dist = sqrt((circBord(1,2) - circCent(1,2))^2 + (circBord(1,1) - circCent(1,1))^2);
      if isnan(dist)
          dist = 1;
      end
      img = getimage(handles.axesCamera1);
      img = insertShape(img,'FilledCircle',[circCent dist],'Color',color,'Opacity',0.3);
      imshow(img,'Parent',handles.axesCamera1);
      drawnow();
    end
  end
  ...
  function finish()
    ...
    color = str{val};
    img = getimage(handles.axesCamera1);
    img = insertShape(imgOrig,'FilledCircle',[circCent dist],'Color',color,'Opacity',0.3);
  end
